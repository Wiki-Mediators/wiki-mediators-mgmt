---
name: "Tight-State Opening Breakout Short"
tagline: "Short a first 2-minute RTH breakdown when SMA20 and SMA200 are compressed."
...
canonical_spec: null
implementation: null
related_candidates: ["tight_open_breakout_long", "tight_problem_bar_removal_short"]
...
admissibility:
  r2_apex_survival:
    ...
    expected_stop_distance_pts_range: [18, 19]
  ...
  r4_signal_frequency:
    ...
    probe_script: "nb_lib/scripts/probe_r4_tight_open_breakout_short.py"
    probe_output: "nb_lib/probe_results/tight_open_breakout_short_r4_probe.json"
    ...
    probe_median_drift_pts: 16.5
    probe_p95_drift_pts: 16.5
...
# Tight-State Opening Breakout Short

## 0. Admissibility summary

- **R1 (edge thesis)**: When the 2-minute SMA20/SMA200 pair is compressed at the RTH open, a bearish first bar below both averages may mark balance breaking lower. The short version mirrors the long setup: a bearish elephant or topping-tail first bar below the cluster triggers a stop entry below bar one. This is a stocks-derived trader-art pattern adapted to MNQ.
- **R2 (Apex survival)**: Uses fixed-risk-dollar sizing at $300 per trade. Expected stop range is pinned to 18-19 points from the R4 fill-geometry probe or generic guard fallback. Worst plausible cluster is 5 consecutive losses = $1500, 0.75 of the Apex 50K EOD $2K drawdown floor. Favorable-first-week independence is false because a sparse opening-window strategy can start with a loss cluster before building cushion.
...
When the 2-minute SMA20/SMA200 pair is compressed at the RTH open, a bearish first bar below both averages may mark balance breaking lower. The short version mirrors the long setup: a bearish elephant or topping-tail first bar below the cluster triggers a stop entry below bar one. This is a stocks-derived trader-art pattern adapted to MNQ.
...
- Compression marks a narrow state where opening flow can produce a directional expansion.
- A bearish first RTH bar below both SMAs suggests sellers are accepting prices under the compressed reference area.
- The stop entry below bar one requires continuation instead of shorting the first print directly.
...
- Require bar1 to be bear_elephant or bear_topping_tail.
- Signal fire price is bar1 low - 0.25 point.
- Initial stop anchor is bar1 high + 0.25 point.
...
This is closest to prior morning breakout failures, especially variant_1_lifecycle and noise_brk_canonical_alpha. Unlike those, it requires tight SMA state and a first 2-minute bar anatomy trigger. That is a real structural distinction, but it is still continuation-class and inherits the 9-fleet warning that MNQ morning breakouts have repeatedly failed Apex-style testing.
