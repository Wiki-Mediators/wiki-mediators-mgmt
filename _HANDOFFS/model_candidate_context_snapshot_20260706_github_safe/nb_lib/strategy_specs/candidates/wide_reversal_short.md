---
name: "Wide-State Short Reversal"
tagline: "Fade an upside wide-state opening trigger back toward the moving-average cluster."
...
canonical_spec: null
implementation: null
related_candidates: ["wide_reversal_long"]
...
admissibility:
  ...
  r4_signal_frequency:
    ...
    probe_script: "nb_lib/scripts/probe_r4_wide_reversal_short.py"
    probe_output: "nb_lib/probe_results/wide_reversal_short_r4_probe.json"
...
# Wide-State Short Reversal

## 0. Admissibility summary

- **R1 (edge thesis)**: When SMA20 is far above SMA200 on 2-minute MNQ bars, the transcript frames the market as wide rather than narrow. In an upside wide state, the candidate does not buy continuation; it waits for a bearish first-bar trigger and shorts for reversion back toward the moving-average cluster. This is a stocks-derived wide-state contrarian rule adapted to MNQ.
...
- A wide upside SMA state marks extended separation rather than fresh compression.
- A bearish elephant or topping-tail first bar suggests opening buyers failed to extend an already wide state.
- The trade attempts to capture flow from wide back toward narrow, targeting SMA20/SMA200 or fixed-R equivalents at spec stage.
...
- At 09:30 ET, require SMA20 > SMA200 and abs(SMA20 - SMA200) / ATR14 >= 1.00.
- Require bar1 close at least 0.50 ATR14 above SMA20.
...
This is a mean-reversion candidate, so the closest rejected relatives are vwap_stretch_snapback and prior_day_value_area_rejection. Unlike VWAP or value-area reversion, the reference state is SMA20/SMA200 expansion. The 9-fleet warns that mean reversion can be Apex-safe but no-edge; this candidate must prove that wide-state context is a better anchor than VWAP or prior-day value.
