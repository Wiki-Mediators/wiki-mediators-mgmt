---
name: "Tight-State Bullish Problem-Bar Removal"
tagline: "Delay a long continuation entry until a wrong-color first bar is removed."
...
admissibility:
  ...
  r4_signal_frequency:
    ...
    probe_script: "nb_lib/scripts/probe_r4_tight_problem_bar_removal_long.py"
    probe_output: "nb_lib/probe_results/tight_problem_bar_removal_long_r4_probe.json"
...
# Tight-State Bullish Problem-Bar Removal

## 0. Admissibility summary

- **R1 (edge thesis)**: In a tight opening state with price above the SMA cluster, a red first bar may be a temporary problem bar rather than a valid reversal. The candidate waits instead of entering immediately; if a later 2-minute bar removes the red bar's high within five bars, it enters long. This is the transcript's coffee-and-wait rule adapted to MNQ.
...
In a tight opening state with price above the SMA cluster, a red first bar may be a temporary problem bar rather than a valid reversal. The candidate waits instead of entering immediately; if a later 2-minute bar removes the red bar's high within five bars, it enters long. This is the transcript's coffee-and-wait rule adapted to MNQ.
...
- The initial red bar represents hesitation or early profit-taking against an above-cluster bullish bias.
- Removal of that red bar's high is treated as evidence that the opening problem was absorbed.
- The delayed entry avoids forcing the clean breakout template when bar one is the wrong color.
...
- Require bar1 close above the higher of SMA20 and SMA200.
- Require bar1 red: close < open.
- Within the next five 2-minute bars, require high >= bar1 high + 0.25 point.
- Signal fire price is bar1 high + 0.25 point; stop anchor is bar1 low - 0.25 point.
...
This variant is more novel relative to the 9-fleet than the clean breakout entries. It is not buying the first continuation bar; it waits for a wrong-color bar to be removed. It still lives in continuation territory, but the anti-continuation setup timing is different from variant_1, noise_brk, PRJ_3, ema_trend, and atr_regime.
