---
name: "Wide-State Long Reversal"
tagline: "Fade a downside wide-state opening trigger back toward the moving-average cluster."
...
canonical_spec: null
implementation: null
related_candidates: ["wide_reversal_short"]
...
admissibility:
  ...
  r4_signal_frequency:
    ...
    probe_script: "nb_lib/scripts/probe_r4_wide_reversal_long.py"
    probe_output: "nb_lib/probe_results/wide_reversal_long_r4_probe.json"
...
# Wide-State Long Reversal

## 0. Admissibility summary

- **R1 (edge thesis)**: When SMA20 is far below SMA200 on 2-minute MNQ bars, the transcript frames the market as downside-wide and vulnerable to reversion back toward narrow. The long version waits for a bullish first-bar trigger in that extended state and enters above the trigger bar. This is the inverse wide-state contrarian rule adapted to MNQ.
...
- A wide downside SMA state marks extended bearish separation.
- A bullish elephant or bottoming-tail first bar suggests sellers failed to extend the already wide downside condition.
- The trade attempts to capture flow from wide back toward the SMA cluster rather than chase breakdown continuation.
...
- Require bar1 close at least 0.50 ATR14 below SMA20.
...
This shares mean-reversion family risk with vwap_stretch_snapback and prior_day_value_area_rejection, but uses wide SMA-state extension rather than VWAP stretch or prior-day auction structure. The 9-fleet context makes the edge prior low; the value of this entry is that it tests a different reversion anchor.
