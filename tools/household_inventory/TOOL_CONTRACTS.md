...
| Pantry Cookbook | Read-only confirmed-pantry × local-recipe-index projection; `recipe_cookbook.py import|state` and `/cookbook.html` | `0` OK; `1` unexpected error; `2` validation/schema/input refusal |
...
| All-Low coverage preview | `POST /api/deals/preview-all-low` with `{"site_id":"HOME"}`. Treats every saved item as hypothetical Low, uses retained HOME search responses only, makes zero network requests, and returns non-acceptable preview rows plus explicit cache misses. |
...
| Draft lifecycle | Pantry edits synchronously update browser-local draft storage and may be recovered only against the same base ledger event count. No elapsed-time, inactivity, page-close, or background process may promote a draft into the ledger. A reminder may call attention to unsaved changes but must not call `POST /api/events`. |
| Local-deals outputs | Dated needs, raw flyer snapshot, normalized offers, pending candidates, ambiguity-review queue with retained clipping URLs when supplied, run manifest, and append-only accepted-shopping-list events under `C:\VMShare\local-deals\`. Cache-only all-Low artifacts use the separate `previews\` branch. No local-deals artifact may be written inside the vault. |
| Flyer request policy | One postal flyer index plus only current-needs item searches, with a config-owned two-second minimum interval. Normal joins refuse bulk flyer-detail fetching. The nearby `N2H2E9` postal code is a debug fixture, not a rotation mechanism. |
| Flyer UI status | The sticky flyer indicator animates only while an operator-requested live fetch or cache-only preview is executing. At rest it reports the retained snapshot timestamp/expiry state. Store listings are grouped per pantry item with deterministic `Ends` or future `Starts … ends …` labels; the 14-day calendar is optional and collapsed by default. **Show all offers** expands retained candidates and review rows locally and must not trigger a network request. |
| Staples watch | `local_deals.config.json` supplies the initial **Staples** watch set. The eye toggle emits append-only `watch-set` events on Save and may enable any catalogue item or explicitly disable a config default. Normal joins include effective watched items regardless of pantry state and deduplicate them against urgent needs. A watch is price discovery, never a pantry-state mutation or automatic shopping-list acceptance. |
| Pantry JSON stock state | Exports write durable `stock_state` (`on-hand`, `low`, `last-meal`, `out`) plus readable `stock_state_label`. The On site summary and category editor change the same draft value; only Save to ledger confirms it. |
...
| Ambiguity boundary | Missing/multiple package sizes remain unranked. Pantry Picker may show retained clippings as responsive tiles and, on explicit click, load one original flyer overview into the in-page viewer, project the selected retained coordinate box, and transition to the clean clipping. Multiple retained clippings for the same pantry item may be reviewed with item-level dots, previous/next controls, or arrow keys; the original retained flyer page for the clipping that opened the viewer is appended as the final slide, and changing slides does not alter matching state. This is an on-demand visual review, not a bulk flyer clone or a new matching signal. A local loopback vision model may later propose visible wording, but neither image display nor model output may promote, rank, accept, or mutate a pantry count without explicit operator confirmation. |
...
## Pantry Cookbook operational contract

| Field | Contract |
|---|---|
| Invocation | Import: `<profile python_executable> tools\household_inventory\pantry_picker\recipe_cookbook.py --profile tools\household_inventory\local_profile.json import [--source-archive PATH] [--foodcom-path PATH] [--index-path PATH] [--summary]`. Query: replace `import` with `state [--site-id HOME] [--query TEXT] [--category TEXT] [--source TEXT] [--max-missing N] [--limit N]`. HTTP clients may repeat `category` and provide `offset` for progressive pages. UI: use **Cookbook** from the connected Pantry Picker; explicit Food.com recovery is `POST /api/cookbook/recover` with one local `recipe_id`. |
| Inputs | Operator-supplied Kaggle 64K recipe CSV or ZIP plus optional Food.com `recipes.parquet`; Pantry Picker catalogue; read-only confirmed pantry projection; optional food/title query, source, multi-category, missing-count, limit, and offset filters. Retained sources are CC0 and live under `C:\VMShare\household-inventory\recipes\`, outside the vault and checked outside configured cloud-sync roots. |
| Outputs | Deterministic `recipe_index.sqlite` under the profile-owned recipe root; paged JSON query projection containing recipe metadata, ingredient availability, missing/unmapped counts, directions, recovery state, and `has_more`/`next_offset`. Verified on-demand Food.com results are one JSON file per source record under the profile-owned `recipes\foodcom_recovered\` cache. `--summary` emits no more than the key import counts plus index path. |
| Deduplication | Exact title+ingredients+directions duplicates collapse within 64K Dishes while preserving category memberships. Food.com records retain their source record IDs. Cross-source records are never silently merged. The 2026-07-28 combined index holds 545,317 recipes: 25,021 from 64K Dishes and 520,296 from Food.com. |
| Matching boundary | Import-time, catalogue-owned phrase matching only. Search suggestions are deterministic local fuzzy matches over retained recipe facets, categories, pantry catalogue names, and representative titles; choosing one searches titles and indexed food-layer values. Known collision exclusions are explicit in code; unmapped wording remains `unknown`. Food.com rows whose quantity and ingredient-name arrays differ retain names only and carry a source warning; quantities are never guessed or positionally forced. On explicit recovery, the server accepts Schema.org ingredient lines only when the Food.com title matches and every retained local ingredient name appears as an ordered normalized subsequence. Failure preserves the source warning and writes no cache. No embedding, model inference, nutritional conclusion, substitution decision, or quantity-sufficiency claim is made. |
| Write boundary | Import may replace only the generated recipe index. Explicit Food.com recovery may atomically create only its profile-owned cache JSON; it never rewrites the index. Cookbook reads and recovery never append pantry events, accept flyer candidates, or mutate the shopping-list ledger. |
| Process exits | `0 OK`; `1 ERROR` for unexpected runtime/infrastructure failure; `2 REFUSED` for missing source, schema drift, or invalid structured input. |
